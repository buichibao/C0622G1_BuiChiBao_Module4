...
public interface ICustomerRepository extends JpaRepository<Customer, Integer> {
    ...
    @Query(value = "select * from customer join customer_type on customer.customer_type_id = customer_type.id where customer.name_customer like %:nameSearch% and customer.email like %:emailSearch% and customer_type.name like %:customerType% and customer.delete_status = 0", nativeQuery = true)
    ...
    @Query(value = "select c.name_customer , atf.name_attach from customer  as c  join contract as ct on ct.customer_id = c.id join contract_detail as ctd on ctd.contract_id = ct.id join attach_facility as atf on atf.id = ctd.attach_facility_id where c.id =:idSearch",nativeQuery = true)
